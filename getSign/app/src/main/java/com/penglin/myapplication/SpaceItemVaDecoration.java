...
public class SpaceItemVaDecoration extends RecyclerView.ItemDecoration {
    ...
    Paint paint;
    int color;
    private int innerPaddingLeftPx =0;//内部的边距

    public void setInnerPaddingLeftPx(int innerPaddingLeftPx) {
        this.innerPaddingLeftPx = innerPaddingLeftPx;
    }
...
//        outRect.left = bottomSpace;
    ...
    public void onDrawOver(Canvas c, RecyclerView parent, RecyclerView.State state) {
        ...
        for ( int i = 0; i < childCount; i++ ) {
            ...
            float dividerLeft = parent.getPaddingLeft()+ innerPaddingLeftPx;
            float dividerBottom = view.getTop();
            float dividerRight = parent.getWidth() - parent.getPaddingRight()-innerPaddingRightPx;
            c.drawRect(dividerLeft,dividerTop,dividerRight,dividerBottom,paint);
        }
    }

    private int innerPaddingRightPx=0;
    //设置右边线条内边距
    public void setInnerPaddingRightPx(int innerPaddingRightPx) {
        this.innerPaddingRightPx=innerPaddingRightPx;
    }
}